...
class Common
{
  ...
  {
    if(preg_match('/^([a-zA-Z0-9]){8,}$/',$this->dataArr['password']) === 0){
      $this->errArr['password'] = 'パスワードは8桁以上の半角英数字で入力してください';
    }
  }

  private function passwordConfirmCheck()
  {
    if($this->dataArr['password'] !== $this->dataArr['passwordConfirm']){
      $this->errArr['passwordConfirm'] = 'パスワードが異なります';
    }
  }
  ...
  private function agreeCheck()
  {
    if(isset($dataArr['agree']) === true){
      if($this->dataArr['agree'] === ''){
        $this->errArr['agree'] = '登録には、個人情報の取扱いに同意が必要です。';
      }
    }
  }
